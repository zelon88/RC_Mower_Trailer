...
module Bearing_Carrier_Single() {
  ...
  difference() {
    ...
    // Cut the center hole through the bottom. This must be cut here too, otherwise the cup body
    ...
    // Starts at cup_depth - 1.5 to ensure overlap with the hollow interior.
...
module Bearing_Carrier_Single_Cap() {
  // Bearing dimensions (metric conversions).
  bearing_id = 6.35;      // 1/4" ID.
  bearing_od = 12.7;      // 1/2" OD.
  bearing_width = 4.76;   // 3/16" width.
